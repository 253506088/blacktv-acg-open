...
<div class="layui-upload">
    ...
    <script type="text/html" id="barDemo">
        <a class="layui-btn layui-btn-xs" lay-event="del1">恢复资源</a>
...
<script>
    layui.use('table', function () {
        ...
        table.render({
            elem: '#test'
            , url: '/blacktv/acg/admin/resources/get/adminDate'
            , toolbar: '#toolbarDemo'
            , title: '管理资源表'
            , page: true
            , cols: [[
            ...
            if (obj.event === 'del0') {
                layer.confirm('请确认是否删除该资源', function (index) {
                    if(data.isDlete != 0 || data.isDlete != "0"){
                        ...
                        ajax("/blacktv/acg/admin/resources/set/isDelete","id=" + data.id + "&isDlete=0");
                        //修改数据表格中的值
                        obj.update({
                            isDlete:'0'
                        });
                ...
                layer.confirm('请确认是否恢复该资源？', function (index) {
                    if(data.isDlete != 1 || data.isDlete != "1"){
                        //ajax发起请求,这里就不在页面里删除本条了
                        ajax("/blacktv/acg/admin/resources/set/isDelete","id=" + data.id + "&isDlete=1");
                        //修改数据表格中的值
                        obj.update({
                            isDlete:'1'
                        });
